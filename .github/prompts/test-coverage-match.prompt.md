...
5. 输出评估结论：
   - 哪些功能点已被覆盖（并指明对应的测试用例）。
   - 哪些功能点缺失正向覆盖（并给出应补充的测试用例描述）。

## 输出格式

- 使用无序列表输出，每一项聚焦一个“功能点”（而不是仅列函数名）。
- 每项格式如下：
  - `- [覆盖/缺失] 文件路径#符号(可选): 功能点描述`
  - 紧跟一个缩进子项：`  - 现有测试: ...`（覆盖项必须列出测试名称/it 描述；缺失项写“无”）
  - 再跟一个缩进子项：`  - 建议补充: ...`（仅对缺失项填写；描述要具体到“输入/操作/断言”）
