...
- [x] 1. 为 Router 模块的错误添加 cause
  - [x] 1.1 修改 `src/router/core/injection.ts` 中 `useRouter()` 的错误
    - 为 `routerNotFound` 错误添加 cause
    - _Requirements: 1.1_
  - [x] 1.2 修改 `src/router/core/create-router.ts` 中 `install()` 的错误
    - 为 `routerDuplicateInstallOnApp` 错误添加 cause
    - _Requirements: 1.1_
  - [x]\* 1.3 为 Router 模块的错误添加单元测试
    - 验证 `useRouter()` 错误包含 cause
    - 验证 `router.install()` 重复安装错误包含 cause
    - _Requirements: 1.1_

- [x] 2. 为 Runtime-Core 模块的错误添加 cause
  - [x] 2.1 修改 `src/runtime-core/provide-inject.ts` 中的错误
    - 为 `provide()` 的 `runtimeCoreProvideOutsideSetup` 错误添加 cause
    - 为 `inject()` 的 `runtimeCoreInjectOutsideSetup` 错误添加 cause
    - _Requirements: 1.2_
  - [x] 2.2 修改 `src/runtime-core/renderer.ts` 中的错误
    - 为 `asContainerKey()` 的 `runtimeCoreInvalidContainer` 错误添加 cause
    - _Requirements: 1.2_
  - [x] 2.3 修改 `src/runtime-core/create-app.ts` 中的错误
    - 为 `app.use()` 的 `runtimeCoreInvalidPlugin` 错误添加 cause
    - _Requirements: 1.2_
  - [x]\* 2.4 为 Runtime-Core 模块的错误添加单元测试
