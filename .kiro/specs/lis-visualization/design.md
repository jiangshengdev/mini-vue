...
当不在第0步时，显示上一步和当前步骤的 Sequence State 和 Predecessors 对比。使用 CSS Grid 4列布局让 `→` 箭头自动对齐：

```
Sequence State:
  上一步:  [1]     → values: [1]
  当前:    [1, 2]  → values: [1, 3]    ← 追加
           ↑ 第2列自动取最宽值，使 → 对齐

Predecessors:
  上一步:  [-1, -1, -1, -1, -1]
  当前:    [-1, -1, 1, -1, -1]         ← 位置 2 变化
```
...
.stateCompareContent {
  ...
  /* 列1: 标签（上一步:/当前:）
     列2: 索引数组 - 自动取两行中较宽的宽度
     列3: → values: 值数组（Predecessors 时为空占位）
     列4: 变化指示器 */
}
```
...
- 变化指示器颜色：`--color-text-hint`（与"上一步:"标签一致）
- 不使用斜体字体
- 变化指示器文本使用简体中文：`← 追加`、`← 替换位置 X`、`（无变化）`
