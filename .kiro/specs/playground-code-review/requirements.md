...
本规范定义了一个针对 `playground/` 目录的代码审查任务系统。Playground 作为框架的试玩与手动验证环境，包含示例组件、视图、控制器和路由配置。审查工作按功能模块分组进行，每个模块生成独立的审查报告，最终汇总所有发现。
...
**User Story:** As a 开发者, I want 将 playground 目录按功能模块分组, so that 审查工作可以模块化进行。
...
1. THE Review_System SHALL 识别以下子文件夹作为独立审查单元：
   ...
   - `styles/` - 样式文件
   - `views/basic/` - 基础示例视图
   - `views/lis-visualization/` - LIS 可视化模块

2. THE Review_System SHALL 将以下顶级文件综合为一个审查单元：
   - `app.tsx` - 应用根组件
   - `main.ts` - 应用入口
   - `views/counter-demo.tsx`
   - `views/home.tsx`
   - `views/not-found.tsx`

3. THE Review_System SHALL 忽略非代码文件（如 `.DS_Store`、静态资源文件）
...
1. WHEN 审查一个子文件夹 THEN THE Review_System SHALL 在 `docs/reviews/playground/` 目录下生成对应的 `{folder-name}.md` 文件
...
**User Story:** As a 开发者, I want 生成一个汇总报告, so that 可以快速了解 playground 整体代码质量状况。

#### Acceptance Criteria

1. WHEN 所有审查单元完成 THEN THE Review_System SHALL 生成 `docs/reviews/playground/summary.md` 汇总报告
...
### Requirement 6: Playground 特定审查标准

**User Story:** As a 开发者, I want 针对 playground 代码应用特定的审查标准, so that 确保示例代码的质量和教学价值。

#### Acceptance Criteria

1. THE Review_System SHALL 检查组件是否正确使用框架的响应式 API（ref、reactive、computed）

2. THE Review_System SHALL 检查 JSX 语法是否符合项目规范

3. THE Review_System SHALL 检查控制器与视图之间的职责分离是否清晰

4. THE Review_System SHALL 检查路由配置是否正确且完整

5. THE Review_System SHALL 检查测试文件是否覆盖关键功能
