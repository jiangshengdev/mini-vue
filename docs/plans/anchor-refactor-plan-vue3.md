...
- Out: SSR/hydration、Teleport/Suspense/Transition 等高级特性、与锚点无关的 diff 性能优化、对外 API 语义变更（除非为贴近 Vue3 必须调整 host options）。
...
[ ] 调整 normalize 策略区分“根卸载”与“内部空渲染”：让组件/children 归一时 `null/undefined/boolean` 变为 `Comment` vnode，但根级 `render(undefined)` 仍保持卸载语义（可通过 `src/runtime-core/renderer.ts` 先判空再 normalize，或拆分 `normalizeVNode`/`normalizeRoot` 两套入口于 `src/runtime-core/normalize.ts`）。
...
## Open questions
- 是否接受对 `createRenderer` 的宿主契约做 breaking change（新增 `nextSibling`/`parentNode`）以换取更贴近 Vue3 的 `move/getNextHostNode`？
- 是否接受“空渲染会产生 Comment DOM 节点”（更接近 Vue3、简化锚点，但 DOM 会更“吵”）？
- `normalize` 层是否要对所有 `null/boolean` 子节点都生成 `Comment`（完全对齐 Vue3），还是只保证组件 root/关键路径有占位以降低 DOM 注释数量？
