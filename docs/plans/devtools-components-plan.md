...
- [x] 只读标记：readonly computed / readonly ref 在 Devtools 中不可编辑，避免误写导致异常。
  - 现状：reactive/readonly proxy 已提供 `__v_isReadonly/__v_raw`（`src/reactivity/internals/base-handlers.ts`）；computed 已补齐 `__v_isReadonly`（`src/reactivity/ref/computed.ts`）；`toRef(readonly(...))` 会通过 ObjectRefImpl 的 `__v_isReadonly` 标记只读（`src/reactivity/ref/impl.ts`）。
  - 风险：Devtools 若把只读 computed 当作可写 ref，写入会触发 computed setter 抛错（mini-vue 使用 `runThrowing` 同步抛出），容易导致 Devtools 后端报错或交互中断。
  - 落地：只读 computed / 只读 ref 会返回 `__v_isReadonly === true`，且 `setupState` 写入路径会拒绝对只读 ref/computed 的写入以确保“不崩”（`src/runtime-core/component/instance.ts`）。
- [x] 状态编辑回写：支持编辑 `ref.value` 与 `reactive` 字段（仅 dev-only；不承诺 Map/Set 等复杂结构）。
  - 现状：`instance.setupState` 已具备 proxyRefs 语义（写入会落到 `ref.value`），并已具备 `component:added/updated/removed` 自动刷新链路；回归用例已覆盖写回与更新（`test/devtools/setup-state-proxy.test.tsx`）。
...
- [x] 回归测试：在 `test/devtools/**` 注入 hook，覆盖“收集去重 / 自动刷新事件 / setup state 代理不崩”。
  - 现状：已覆盖 setup state 命名（`test/devtools/setup-state-names.test.tsx`）、`app:init` 后回填（`test/devtools/component-picker-backfill.test.tsx`），并新增覆盖去重/写回/只读保护（`test/devtools/setup-state-proxy.test.tsx`）与 updated/removed 事件（`test/devtools/component-events.test.tsx`）。
