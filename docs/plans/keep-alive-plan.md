...
[x] 扩展运行时 vnode 元数据：为 keep-alive 增加最小字段（如 `shouldKeepAlive/keptAlive/keepAliveInstance`）与类型/守卫（`src/runtime-core/virtual-node.ts` 等）。  
[x] 扩展组件生命周期：新增 `onActivated/onDeactivated` 注册 API、实例存储结构与 post flush 调度（`src/runtime-core/component/context.ts`、`src/runtime-core/component/lifecycle.ts`、`src/runtime-core/component/index.ts`、`src/runtime-core/index.ts`、`src/index.ts`）。  
[x] 实现 `KeepAlive` 组件本体：维护 `cache: Map` + `keys: Set`（LRU），支持最小 `max/include/exclude`（可拆阶段），并在渲染期为子组件 vnode 写入 keep-alive 标记（建议新增 `src/runtime-core/components/keep-alive.ts` + `src/runtime-core/components/index.ts`）。  
[x] 在 `mount` 路径支持 `keptAlive`：命中缓存时不新建实例，改为调用 `activate`（`move` 回实际容器 + 入队 activated hooks），并保持 `handle/nodes` 可复用（`src/runtime-core/mount/virtual-node.ts` 等）。  
