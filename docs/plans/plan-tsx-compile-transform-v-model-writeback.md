...
- 需要明确：是否缓存 key（render 阶段缓存 vs 事件触发时实时读取）、是否允许 base/key 有副作用（默认先拒绝）。
...
- 明确：事件触发时写回使用的是“哪一轮 render 生成的 handler”（依赖 props patch 更新语义）。  
  [ ] 补齐落盘与互链：在 `docs/issues/jsx-runtime-issues.md` 更新“非 Ref 目标不写回”的约束与推荐方案，并在 `docs/plans/plan-v-model-jsx.md` / `docs/plans/plan-v-model-component.md` 互链该计划。  
...
- P1 动态 key 的默认策略选哪个：
  - A) “实时读取 key”：闭包里用 `obj[key] = $event`；
  - B) “缓存 key”：render 阶段缓存 `const __k = key`，闭包写 `obj[__k] = $event`？
- DOM 表单的 TSX 编译期是否也一并展开到 `value/checked` + `onInput/onChange`，还是继续让 `runtime-dom` 消费 `'v-model'` 做兜底？
