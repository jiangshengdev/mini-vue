...
- [x] 实现转换入口：在 `runtime-dom/props/patch-props.ts` 消费 `'v-model'`，生成等价的 value/checked + handler，并移除原始 `'v-model'`；默认使用严格等于判定（不做松散相等）。
- [x] 补充类型：在 `src/jsx-shim.d.ts` 等声明中为表单标签补齐 `'v-model'` 签名；组件 v-model 支持见 `docs/plans/plan-v-model-component.md`。
...
- 已迁移到宿主层（`runtime-dom`），并移除 `jsx-runtime` 的 DOM 转换；因此只要使用 `runtime-dom`，不管 JSX 还是手写 `h` 都能使用 DOM 表单 `v-model`。
