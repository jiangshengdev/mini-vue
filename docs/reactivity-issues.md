...
- 位置：`src/reactivity/internals/baseHandlers.ts:8-39`
...
## 3. 非普通对象（Map/Set/Date 等）被意外代理（已修复）

- 位置：`src/reactivity/reactive.ts:34-55`
- 问题：当前只对数组显式报错，其它内建对象会直接进入 `Proxy`。但配套的 handler 只适用于普通对象：例如 `reactive(new Map())` 后执行 `state.set()` 不会触发 `trigger`，因为 handler 没有拦截 `Map` 内部的数据读写，导致响应式静默失效。
- 影响：开发者误用非 Plain Object 时不会收到任何提示，应用表面“正常”但所有 effect 永远不会更新，调试成本极高；同时与文档“仅支持普通对象”的声明不符。
- 建议：在 `reactive` 入口增加 Plain Object 判定（如 `Object.getPrototypeOf(target) === Object.prototype`），或在 README 中明确声明并新增断言/报错，防止不受支持的类型被静默代理。
- 解决：`src/shared/utils.ts` 新增 `isPlainObject`，`reactive.ts` 根据该工具拒绝数组外的所有非普通对象；`test/reactivity/reactive.test.ts` 增补 Map/Set/Date 场景确保抛出统一错误。
- 验证：`pnpm test reactive.test.ts`
