...
## 1. effect.stop 缺乏任何断言（待补充）

- 位置：`test/reactivity/effect.test.ts`
- 问题：当前用例只验证副作用会立即执行、可通过返回句柄 `run()` 重放，以及依赖切换场景，但没有任何一条用例覆盖 `handle.stop()`。一旦 `ReactiveEffect.stop()` 的依赖清理逻辑或 `effectScope` 的嵌套清理出现回归，测试套件完全无法察觉。
- 影响：手动停止 effect（例如在组件卸载或 HMR dispose 时）是响应式系统的核心能力之一，缺少测试等于允许内存泄漏、重复触发、状态“幽灵”问题悄然上线。
- 建议：新增场景：调用 `effect` 返回的句柄，修改响应式对象，验证 effect 会触发；随后调用 `handle.stop()` 再修改相同字段，应确保副作用不再执行，并断言 `handle.run()` 在停止后保持惰性（不重新收集依赖）。
...
- 位置：`test/jsx` 目录（所有用例）
- 问题：`src/jsx/renderer/props.ts` 支持函数式 `ref`，包括挂载时拿到 DOM、卸载时回调 `null`。然而测试完全未覆盖 `ref` 绑定，连最基本的“渲染后 ref 收到元素”都没有，更遑论在 `render` 二次调用或 `createApp.unmount()` 时重新触发清理。
