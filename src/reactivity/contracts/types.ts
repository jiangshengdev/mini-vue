/**
 * 汇总响应式系统的核心契约类型，覆盖 effect 接口与代理内部标记。
 *
 * @remarks
 * - 统一抽象生命周期与依赖管理语义，使运行时实现与类型校验保持一致。
 * - 区分外部 handle 与内部 instance 角色，约束可见能力与职责。
 */
...
export interface ReactiveProxyInternals {
  ...
  /** 标记对象为浅层代理，供内部逻辑区分 shallow/深层语义。 */
  readonly [shallowFlag]?: true
