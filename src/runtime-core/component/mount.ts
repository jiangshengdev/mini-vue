...
>(
  ...
  const instance = createComponentInstance<HostNode, HostElement, HostFragment, T>(
    component,
    props,
    container,
    {
  ...
  /* 执行首次渲染并将子树挂载到宿主容器。 */
  const initialRender = performInitialRender<HostNode, HostElement, HostFragment, T>(
    options,
    instance,
  )
